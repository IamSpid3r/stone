...
exports.getInfo = function(urlStr, callback) {
    ...
    if(urlInfo.host == 'store.nike.com'){
        var exp_normal = /cn\/zh_cn\/pd.*pgid-(\d*)/ig;            //正常详情页
        var exp_custom = /cn\/zh_cn\/product.*piid=(\d*)/ig;      //定制详情页

        res_normal = exp_normal.exec(urlInfo.path);
        res_custom = exp_custom.exec(urlInfo.path);

        if(res_normal){
            getNormalItemInfo(urlStr, callback);
        }else if(res_custom){
...
*正常内容处理
**/
function getNormalItemInfo(urlStr,callback) {
