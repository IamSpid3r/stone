...
async function getNormal2ItemInfo(urlStr, callback, res_normal2) {
    try {
        //获取html
        var body = await getHtml2(urlStr);
        ...
        if (!resSku) {
            let errMsg = body.indexOf('iframe') != -1 ? 'iframe sick' : 'Not found sku info';
            throw new Error(errMsg);
        }

        var skuInfo = JSON.parse(resSku[1]);
        var n = j = i = h = f = 0;
        ...
        // })
        // if (styleLongIds.length > 5) {
        //     let stockUrl = 'https://api.nike.com/deliver/available_skus/v1/?filter=productIds';
        //     stockUrl += '('+styleLongIds.join(',')+')';
        // }
        //
        // let stockBody = await getHtml2(stockUrl);
        ...
        if ('Threads' in skuInfo) {
            _(skuInfo.Threads.products).forEach(function (product) {
                ...
                var skuStateArr = [];
                product.availableSkus.forEach(function (availableSku) {
                    skuStateArr[availableSku.id] = true;
                })
