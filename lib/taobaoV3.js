...
function getItemInfo(urlInfo, callback) {
    ...
    superRequest({url: apiUrl}, function (err, response, body) {
        ...
        if (!('seller' in responseFirstJson.data)) {
            var itemInfo = {
                Unique: 'cn.taobao.' + urlInfo.query.id,
                Md5: '',
                Status: 'outOfStock',
        ...
        try {
            var sellerType = responseFirstJson.data.seller.sellerType;
            var itemInfo = {
                Unique: 'cn.taobao.' + urlInfo.query.id,
                Md5: '',
                Status: 'inStock',
            ...
            console.error(e.message + urlInfo.query.id)
            return callback({
                "Errors":{
                    'Code': 'Error',
                    "Message": e.message + urlInfo.query.id
                }
            });
        }
        ...
        //津贴
        if (
            ('coupon' in apiStackValue.resource && 'couponList' in apiStackValue.resource.coupon)
            || ('entrances' in apiStackValue.resource && 'coupon' in apiStackValue.resource.entrances)
        ) {
            if ('couponList' in apiStackValue.resource.coupon) {
                var couponListJieTie = apiStackValue.resource.coupon.couponList;
            } else {
                var couponListJieTie = apiStackValue.resource.entrances.coupon;
            }
            if (!Array.isArray(couponListJieTie)) {
                couponListJieTie = [couponListJieTie];
            }
            couponListJieTie.forEach(function (couponVal) {
                var textFee = /领津贴每满(\d*)减(\d*)/.exec(couponVal.text);
                if (textFee) {
                    Jintie = [
                        {
                            Id: null,
        ...
        if ('coupon' in apiStackValue.resource && 'shopProm' in apiStackValue.resource) {
            apiStackValue.resource.shopProm.forEach(function (couponVal) {
                ...
                if (couponVal.iconText == '跨店满减'
                    ...
                    let startTime = '2018-12-12 00:00:00';
                    let endTime = '2018-12-12 23:59:59';
                    kuadian = [
                        {
                            Id : null,
        ...
        if ('delivery' in apiStackValue && 'postage' in apiStackValue.delivery) {
            ...
            if (postageRes = postageReg.exec(apiStackValue.delivery.postage)) {
                if (Number(postageRes[1]) > 0) {
                    ...
                    postage = 10;//todo 为了不同区域代理ip快递费一致，采用10
                }
            }
        }
        //服务
        ...
        (async () => {
            try {
                ...
                var coupon = await crawlCoupon(urlStr, shopId, userId, sellerType);
                if (coupon.Status) {
...
//获取优惠券
var crawlCoupon = function (urlStr, shopId, sellerId, sellerType) {
    return new Promise((resolve, reject) => {
        let merchantTicketUrl = merchantTicket + '?url=' + encodeURIComponent(urlStr)
            + '&shopId=' + shopId + '&sellerId=' + sellerId+ '&sellerType=' + sellerType + '&v=v2';
        request({
...
var superRequest = function (options, callback) {
    ...
    proxyRequest2(options, function (error, response, body) {
        if (error) {
            console.log('v3 again proxyRequest', body)
            proxyRequest2(options, function (error, response, body) {
