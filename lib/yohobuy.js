...
exports.getInfo = function(urlStr, callback) {
    ...
    try {
        if(urlInfo.host == 'item.yohobuy.com'){
            if(urlInfo.path.split('/').length == 2){
                var exp = /p(\d*)\./ig;
    ...
    var mApi = 'https://m.yohobuy.com/product/detail/info';
...
function getItemInfo(params, callback) {
    ...
    if(cartInfo.hasOwnProperty('thumbs') && cartInfo.thumbs.length > 0){
        img = cartInfo.thumbs[0].img;
        index = img.indexOf('?')
    ...
    if(cartInfo.hasOwnProperty('colors') && cartInfo.colors.length > 0){
        cartInfo.colors[0].color.forEach(function(val, index){
            img = cartInfo.thumbs[index].img;
            index = img.indexOf('?')
    ...
    if(cartInfo.hasOwnProperty('sizes') && cartInfo.sizes.length > 0){
        cartInfo.sizes[0].size.forEach(function(val, index){
            size.Values.push({
                "ValueId": val.id,
                "Name": val.name,
            });
    ...
    if(cartInfo.hasOwnProperty('sizes')){
        cartInfo.sizes.forEach(function (val,index) {
            if(index > 0){
                val.size.forEach(function (valDetail) {
                    if(valDetail.sizeNum > 0){
                        colorIndex = _.findIndex(color.Values, {'ValueId':valDetail.colorId});
                        sizeIndex = _.findIndex(size.Values, {'ValueId':valDetail.id});

                        //保存商品信息
                        itemContent = {
                            "Unique":"cn.yohobuy."+valDetail.skuId,
                            "Attr":[
                                {
                                    "Nid": color.Id,
                                    "N":   color.Name,
