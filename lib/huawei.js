        ...
        try {
            ...
            if (couponList.length > 0) {
                ...
                couponList.forEach(function (couponVal) {
                    //满减加金额未出现过
                    if (couponVal.couponType == 1 && (-1 == _.findIndex(couponDetail, {
                            batchCode : couponVal.batchCode, activityCode:couponVal.activityCode}
                    ))) {
                        couponDetail.push(couponVal);
                    }
                })
                if (couponDetail) {
                    couponDetail.forEach(function (couponDetailVal) {
                        var stime = fun.dateformat(new Date(couponDetailVal.beginDate), 'yyyy-MM-dd hh:mm:ss');
                        var etime = fun.dateformat(new Date(couponDetailVal.endDate), 'yyyy-MM-dd hh:mm:ss');
                        itemInfo.Coupon.List.push({
                            "Id": couponDetailVal.activityCode,
                            "Amount": [couponDetailVal.amount, couponDetailVal.amtMin],
                            "Date": [stime, etime],
                            "Category": "normal",
                            "Type": "item"
                        });
