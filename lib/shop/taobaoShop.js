...
console.log(apiUrl);
    proxyRequest({
        ...
        var items =[];
        var totalResults = Math.ceil(text.total_results / 24)
        ...
        if(pageNum > 0){
            proxyRequest({
                url: 'http://hws.m.taobao.com/cache/wdetail/5.0/?id=' +text.items[0].item_id+ '&ttid=2013@taobao_h5_1.0.0&exParams={}',
                headers: { 'User-Agent' : 'Mozilla/5.0 (iPhone; CPU iPhone OS 7_0 like Mac OS X; en-us) AppleWebKit/537.51.1 (KHTML, like Gecko) Version/7.0 Mobile/11A465 Safari/9537.53'}
            }, function(error, body) {
                try{
                    var responseFirstJson = JSON.parse(body);
                    if(responseFirstJson.ret[0] == "SUCCESS::调用成功"
                        && ('itemTypeName' in responseFirstJson['data']['itemInfoModel'])
                        && responseFirstJson['data']['itemInfoModel']['itemTypeName'] == 'tmall'
                    ){
                        store = 'tmall';
                    }
                }catch(exception){
        ...
        ep.after('currentUrls', urls.length, function (currentItems) {
            ...
            currentItems.forEach(function(currentItem){
                ...
                currentItem.items.forEach(function(item) {
                    ...
                    if(item.title.indexOf('一元竞拍') < 0||item.title.indexOf('一元起竞拍') < 0||item.title.indexOf('一元公益竞拍') < 0){
                        items.push({
                            Unique: 'cn.taobao.' + item.item_id,
                            Title: item.title,
                            Img: 'http:'+item.img,
                            Url: getRealUrl(item.url, store),
