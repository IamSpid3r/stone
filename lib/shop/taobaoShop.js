...
function getItemInfo(params, callback){
    ...
    proxyRequest({
       ...
       /* callback(null,body);
        ...
        var items =[];
        var pageNum = ( text.total_page - page) > 2 ? (page+2) : text.total_page; //每次抓取的终止页数
        var urls = [];
        for(var i=(page+1); i<=pageNum; i++){
            urls.push('https://xmofs.m.tmall.com/shop/shop_auction_search.do?' +
            'suid='+userId+'&sort=s&p='+i+'&page_size=24&from=h5&shop_id='+shopId+'&ajson=1&_tm_source=tmallsearch&callback=json');
        }

        //页面抓取信息
        itemInfo.ItemAttributes.TotalPage = text.total_page;
