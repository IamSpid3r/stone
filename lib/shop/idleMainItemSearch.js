...
exports.doSearch = function( params , callback){
    switch(params.source){
        ...
        case '2':
            proxyRequest({
                url: 'https://so.m.jd.com/ware/search._m2wq_list?keyword='+encodeURIComponent(params.keyword) +'&datatype=1&callback=jdSearchResultBkCbB&page='+params.page+'&pagesize=10&ext_attr=no&brand_col=no&price_col=no&color_col=no&size_col=no&ext_attr_sort=no&merge_sku=yes&multi_suppliers=yes&area_ids=12,978,4459&qp_disable=no&t1=1544491605657',
                headers: { 'User-Agent' : 'Mozilla/5.0 (iPhone; CPU iPhone OS 11_0 like Mac OS X) AppleWebKit/604.1.38 (KHTML, like Gecko) Version/11.0 Mobile/15A372 Safari/604.1'}
            },function(err, body){
                if(err){
                    callback(err);
                }
                let text = _.trim(body,'jdSearchResultBkCbB ');
                text = eval(text);
                callback(null,text);
            });
