...
function getItemInfo(params, callback) {
    ...
    var imageUrl = $("#imageMenu>ul>li>a>img").attr("src"),
    	imageUrl = imageUrl.split('_')[0] + '.png';
	...
	$(".size-nav").find("li").each(function(index,obj){
        ...
        n++;
        sizeValueId = type.size+_.padStart(n, 6, 0);
        size.Values.push({
            "ValueId": sizeValueId,
            "Name": sizeName
        });
	...
	$(".color-size-category").find("li").each(function(index,obj){
		...
		if (colorcode == gid) {
			var imgsAttr = [];
			$("#imageMenu ul li a img").each(function(index,obj){
				var tmp = $(obj).attr('src');
				imgsAttr.push(_.replace(tmp, '90X105', '615X650'));
			});
        	color.Values.push({
                "ValueId": itemid,
                "Name": colorName,
                ...
                if ((0 < qyt) && (colorItemid == itemid)) {
                    itemInfo.Items.push({
                        ...
                        "Offers": [{
                            ...
                            "List":[
                                {
                                    "Price": _.trimStart(trim($('.price-style .price').text()),'￥'),
                                    "Type": "RMB"
                                }
                            ]
                        }]
