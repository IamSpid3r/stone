...
exports.getInfo = function(urlStr, callback) {
    ...
    try {
        if(urlInfo.host == 'www.xtep.com.cn' && urlInfo.pathname == '/goods/do_index'){
        	var pid = urlInfo.query.goods_sn;
        }else if(urlInfo.host == 'www.xtep.com.cn' && urlInfo.pathname.indexOf('.html')){
            var  patt = /(spike|goods)-(\d*?)\.html/;
...
function getItemInfo(params, callback) {
    ...
    if(body.indexOf('pro-box') == -1){
        callback('丢失 pro-box');
        return ;
    }
    if (body.indexOf('已售罄') != -1) {
        var itemInfo = {
            Unique: 'cn.xtep.' + pid,
            Md5: '',
            Status: 'outOfStock',
...
/*    var regExp = /var select\_sku\_str \= new Array\(\);([\s|\S]*?)\<\/script\>/g
    ...
    patt = /(select_sku_str[\s|\S]*?)\<\/script\>/ig;
    result = patt.exec(params.body);
    if (result) {
        try {
            eval(result[1]);
        } catch (exception) {
            callback('all_inventory_skus error');
            return;
        }
...
function getHtml(urlStr, callback){
    proxyRequest({
        ...
        if(!error){
            ...
            } else if (body.indexOf('<iframe id="_if"') > 0) {
                callbackStatus(false);
            }  else {
                callbackStatus(true)
            }
