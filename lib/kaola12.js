...
function getItemInfo(params, callback) {
        ...
        if (body.body.couponDetailList != null && body.body.couponDetailList.length > 0){
            try{
                body.body.couponDetailList.forEach(function(coupon){
                    ...
                    if (couponDate.length ==1 && coupon.couponUseTime.indexOf("自领取日开始") > -1){
                        var day = coupon.condition.replace(/自领取日开始/g, "").replace(/天有效/g, "");
                        var now = new Date();
                        ...
                        now.setTime(now.getTime()+24*60*60*1000*10);
                        ...
                        startDate = couponDate[0].replace(/\./g, "-")+" 00:00:00";
                        endDate = couponDate[1].replace(/\./g, "-")+" 23:59:59";
                    }
    ...
    getHtml('https://m.kaola.com/product/queryPromotionNew.html?goodsId='+goods_id+'&t='+new Date().getTime()+'&categoryId=0&provinceCode=310000&cityCode=310100&districtCode=310101', function(body, err){
        ...
        if (body.body.goodsPromotionResult4WapNew != undefined){
            ...
            if(body.body.goodsPromotionResult4WapNew.goodsPromotionResult4Wap.memberCurrentPrice != null){
            ...
            itemInfo.Items.forEach(function(item,row_index){
                itemInfo.Items[row_index].Offers[0].Subtitle = promotion;
            })
        }
        ep.emit('info',[]);
    })
