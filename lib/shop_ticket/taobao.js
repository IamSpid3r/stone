...
exports.getInfo = function(urlstr,  itemId, callback) {
    ...
    if (/(taobao|tmall)\.com/.exec(urlInfo.host)) {
...
function getItemInfo(urlstr, itemId, callback){
    Q('uid').then(function () {
        ...
        proxyRequest({url: urlstr}, function (err, body) {
            if(err){
                defer.reject(err);
            }else{
                defer.resolve(body);
        ...
        if(res1){
            ...
            defer.reject('host error is not taobao shop hostname');
        }
...
function isAlimama(itemInfo, itemId, callback) {
    if(itemInfo.Status) {
        ...
        proxyRequest({url: url}, function (err, body) {
            if(err){
                callback({
                    "Errors":{'Code': 'Error', "Message": err}
                })
