...
function getItemInfo(params, callback) {
    ...
    itemInfo.ItemAttributes.ImageUrl =   'http:'+body.skuImg[sid];
    ...
    try{
        ...
        if (body.attr.attrList.salesPropertyList.length == 0){
            ...
            var wap_url = "http://item.m.gome.com.cn/product/stock?goodsNo="+gid+"&skuID="+body.skuID+"&shopId=&shopType=&provinceId=21000000&cityId=21010000&districtId=21010100&townId=210101006&modelId=&stid=&mid=&isFirst=Y&isPresent=0&ajax=1&_=1495270717971";
            ...
            body.attr.attrList.salesPropertyList.forEach(function(val,key){
                ...
                val.attrList.forEach(function(aval){
                    if (val.attrName == '颜色'){
                        attrTmp.Values.push({
                            "ValueId": aval.weight,
                            "Name": aval.title,
                ...
                items.forEach(function(item){
                 ...
                 if (attrArr.length > 0){
                    attrArr.forEach(function(attr){
                        itemInfo.Variations.push(Variations[attr]);
                    })
                 }
                itemInfo.Md5 = md5(JSON.stringify(itemInfo));
                callback(null, itemInfo);
            ...
            priceSkus.forEach(function (priceSku) {
                var wap_url = "http://item.m.gome.com.cn/product/stock?goodsNo="+gid+"&skuID="+priceSku+"&shopId=&shopType=&provinceId=21000000&cityId=21010000&districtId=21010100&townId=210101006&modelId=&stid=&mid=&isFirst=Y&isPresent=0&ajax=1&_=1495270717971";
                getHtml(wap_url, function(bodyPrices, err){
                    if(err){
                        callback({
                            "Errors":{
