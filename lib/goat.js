...
class Goat {
    ...
    async getItemInfo() {
        ...
        let data = await this._getJson(slug)

        if (data.hasOwnProperty('errors')) {
            this._itemInfo.Status = 'notFind'
        } else {
            data = data.data.viewer.productTemplate

            if (data.nickname == null) {
                data.nickname = data.name
            }

            this._updateBaseInfo(data)

            this._addVariations(data)
    ...
    _updateBaseInfo(data) {
        const women = 'Women\'s'
        const men = 'Men\'s'

        this._itemInfo.Unique = `${country}.${platform}.${data.id}`

        this._itemInfo.ItemAttributes.ShopId = `${country}.${platform}`

        this._itemInfo.ItemAttributes.Title = `${data.brand.name} ${data.name}`
    ...
    _addVariations(data) {
        ...
        let urls = data.product_template_additional_pictures.map((item) => {
            return item.original_picture_url
        })

        urls.push(this._itemInfo.ItemAttributes.ImageUrl)
        ...
        this._itemInfo.Variations.push({
            ...
            Values: [{
                ValueId: 1,
                Name: data.sku ? data.sku : data.name,
                ImageUrls: urls,
            }],
        })
        ...
        const values = data.formatted_available_sizes_new_v2.map((item) => {
            return {
                ...
                Name: parseInt(item.size) + gender,
            }
        })
