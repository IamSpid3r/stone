    ...
    (async () => {
        try {
            ...
            if(urlInfo.host == 'www.6pm.com'){
                ...
                getItemInfo({res:body, urlStr:urlStr}, callback);
            } else {
...
function getItemInfo(params, callback) {
    ...
    try {
        ...
        var productcCategory = __INITIAL_STATE__.pixelServer.data.trackingPayload.product.category;
        var gender = product.detail.genders[0];
            ...
            if (colorIndex > -1) {
                val.stocks.forEach(function (sizeVal) {
                    if((sizeIndex = _.findIndex(size.Values, {ValueId: sizeVal.sizeId})) == -1){
                        if (productcCategory != 'Shoes') {
                            var sizeName = sizeVal.size;
                        } else {
                            var sizeName = gender+' '+sizeVal.size;
                        }
