    ...
    getHtml('https://unszacs.m.taobao.com/h5/mtop.taobao.detail.getdetail/6.0/?api=mtop.taobao.detail.getdetail&v=6.0&ttid=2013%40taobao_h5_1.0.0&type=jsonp&dataType=json&data=%7B%22itemNumId%22%3A%22'+urlInfo.query.id +'%22%7D', function(body ,err){
        ...
        try {
            ...
            if(data.hasOwnProperty('apiStack')){
                ...
                if(apiStackValue.price.hasOwnProperty('extraPrices')){
                    apiStackValue.price.extraPrices.forEach(function (extraPrice) {
                        if(extraPrice.priceTitle == '双11狂欢价'){
                            itemInfo.Price.Text = extraPrice.priceText;
                            itemInfo.Price.Status = true;
                        }
                    })
                }

                if(
                    apiStackValue.resource.hasOwnProperty('entrances')
                    && apiStackValue.resource.entrances.hasOwnProperty('tmallCoupon')
                ){
                    itemInfo.Coupon.Text = apiStackValue.resource.entrances.tmallCoupon.text;
                    itemInfo.Coupon.Status = true;
                }
                if(apiStackValue.resource.hasOwnProperty('shopProm')){
                    apiStackValue.resource.shopProm.forEach(function (shopProm) {
                        if(shopProm.iconText == '满减'){
                            itemInfo.Coupon.Text = shopProm.content[0];
                            itemInfo.Coupon.Status = true;
                        }
                    })
                }
