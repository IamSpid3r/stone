...
function getItemInfo(urlInfo, callback){
    ...
    //拼装评论url
    var comment_url = 'https://m.kaola.com.hk/wapGoods/commentAjax/comment_list.html?goodsId=' + goods_id + '&pageNo=3&pageSize=10&tagType=0';
    //通过代理 获取url
    proxyRequest({
        url: comment_url
    },function(err, body){
        ...
        //拼装返回数据
        var itemInfo = {
            Unique: 'com.kaola.'+goods_id,  //店铺数据
            Status: 'inStock',          //状态
            Url: urlInfo.href,              //url
            ShopUrl : 'http://www.kaola.com/',  //店铺链接
            Comments: []            //评论数组 默认是空
        };
...
function parseComment(commentData) {
    ...
    if(typeof (currentData.comments) == undefined) throw Error('json 内容不正确');
    var comments = [];
