...
exports.getInfo = function(urlStr, callback) {
    ...
    try {
        if(urlInfo.host == 'www.yougou.com' || urlInfo.host == 'seoul.yougou.com'){
            ...
            var sid = sku[1];       //商品id
            var pid = _.trimEnd(sku[2],'.shtml');       //商品子ID
        }else{
            throw new Error();
        }
    } catch (exception) {
...
function getItemInfo(params, callback) {
    ...
    var imageUrl = $('.pro_img img').eq(0).attr('src') != 'http://m.yougou.com:80/images/detail_img/car1.png'
                ? $('.pro_img img').eq(0).attr('src') : $('.pro_img img').eq(1).attr('src');
    ...
    $('.opt_color dd a').each(function(){
        if(!$(this).hasClass('checked')){
            var mUrl = 'http://m.yougou.com/touch/c-newbalance/sku-'+params.sid+'-'+$(this).attr('proNumber');
            $(this).attr('href') != '#' && yougouUrls.push(mUrl)
        }
    })
    ...
    ep.after('yougouUrls', yougouUrls.length, function (yougouItems) {
          yougouItems.push($);
          yougouItems.sort(function(a,b){//异步抓取顺序不同，保持排序规则，保持MD5一致
              return a('.opt_color dd .checked img').attr('alt') > b('.opt_color dd .checked img').attr('alt');
          })

          yougouItems.forEach(function($){
              //获取所有的颜色
              j++;
              var ImageUrls = [];
                  ...
                  if(!$(this).find('span').hasClass('no')){
                      ...
                      itemContent = {
                          ...
                          "Offers": [{
                              ...
                              "List":[
                                  {
                                      "Price": _.trimStart($('.pro_sku .price').text(),'¥'),
                                      "Type": "RMB"
                                  }
                              ]
                          }]
    ...
    yougouUrls.forEach(function (yougouUrl) {
        //console.log(yougouUrl)
        getHtml(yougouUrl, function(body, err){
            if(err){
                callback({
                    "Errors":{
                        'Code': 'Error',
            ...
            ep.emit('yougouUrls', cheerio.load(body));
        });
    })
