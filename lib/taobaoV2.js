    ...
    taobaoToken(params , cookiePath, function (body, err) {
                    ...
                    Jintie = [
                        {
                            ...
                            Date: ['2018-09-09 00:00:00', '2018-09-10 23:59:59'],
                            Category : 'none',
                            Args : {
                                ApplyText: '购物津贴',
        ...
        if ('coupon' in apiStackValue.resource && 'shopProm' in apiStackValue.resource) {
            apiStackValue.resource.shopProm.forEach(function (couponVal) {
                var regexp2 = /([\d\/]*)-([\d\/]*)每满(\d*)减(\d*)/;
                if (couponVal.iconText == '跨店满减'
                    && (textFee2 = regexp2.exec(couponVal.content[0]))
                ) {
                    let startFee = textFee2[3];
                    let amount = textFee2[4];
                    let startTime = (new Date()).getFullYear()+'-'+textFee2[1].replace('/','-')+' 00:00:00'
                    let endTime = (new Date()).getFullYear()+'-'+textFee2[2].replace('/','-')+' 00:00:00'
                    manjian = [
                        {
                            Id : null,
                    ...
                    //唯一id
                    manjian[0].Id = md5(startFee+amount+startTime+endTime);

                    itemInfo.Jintie = manjian;
                }
            })
        }
