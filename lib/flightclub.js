...
const addColorVariation = ($) => {
    ...
    const sku = $('.product-attribute-list .attribute-list-item:nth-child(1)').text().trim()
    ...
    itemInfo.Variations.push({
        ...
        Values: [{
            ValueId: 1,
            Name: sku != '' ? sku : itemInfo.ItemAttributes.Title,
            ImageUrls: [],
        }],
    })
        ...
        if (price > 0) {
            const data = {
                ...
                Attr: [{
                    ...
                    N: '颜色',
                    Vid: 1,
                    V: itemInfo.Variations[0].Values[0].Name,
                }, {
...
const getItemInfo = async (urlStr) => {
    ...
    const title = $('.product-essential .product-shop .product-name h1').text().trim()

    if (title.indexOf(brand) === 0) {
        itemInfo.ItemAttributes.Title = title
    } else {
        itemInfo.ItemAttributes.Title = `${brand} ${title}`
    }

    itemInfo.ItemAttributes.ImageUrl = $('.product-img-box .product-img').data('src')

    addColorVariation($)
    addSizeVariation($, body)
