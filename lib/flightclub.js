...
const addSizeVariation = ($, name, body) => {
    ...
    $('#new-options>ul>li').each((sizeKey, li) => {
        ...
        let Price = /.*?(\d+).*?/.exec(json[Object.keys(json)[0]][button.data('optionid')].price)[1]
        
        if (Price <= 0) {
            Price = /.*?(\d+).*?/.exec(json[Object.keys(json)[1]][button.data('optionid')].price)[1]
        }
...
const getItemInfo = async (urlStr) => {
    ...
    const Unique = `${country}.${platform}.${id}`
