...
exports.getInfo = function (urlStr, callback) {
    ...
    if (urlInfo.host == 'www.windeln.com.cn') {
...
function getItemInfo(params, callback) {
    ...
    pid = $('.product-description-ean').text().replace('EAN:','').trim();
    var title = $('.product-selection')find('h1').text();
    var price = $('.rmb-price').text();
    if (price) {
        price = price.replace('￥', '').trim();
    }
    var img = $('.more-img-container').find('img').attr('src');
