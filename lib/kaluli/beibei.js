   ...
   /* var stockMap = res.sku.sku_stock_map;
    if(typeof stockMap != 'undefined') {
        ...
        for(var key in stockMap){
            ...
            // 将k拆分
            var regexp = /(v.*)(v.*)/;
            var NameId = regexp.exec(key);

            var cc = kvMap[NameId[1]]

            attrs.Id = key
            attrs.NameId = NameId
            stockMapId.push(attrs);
        }
        itemInfo.ItemAttributes.ShopIdss = stockMapId
...
*/
    ...
    /*if(typeof idMap != 'undefined'){//没有sku
        ...
        for(var key in idMap){
            ...
            if(k.length == 2){
                var type1 = [];
                for(var i = 0; i< idMap[k[0]].length; i++){
                    type1.push(idMap[k[0]][i])
                }
                var type2 = [];
                for(var i = 0; i< idMap[k[1]].length; i++){
                    type2.push(idMap[k[1]][i])
                }
    ...
    if(typeof stockMap != 'undefined' && length == 2){
        ...
        for(var i = 0; i<typeOne.length; i++){
            var attrs = {};
            attrs.Attr = [];
            Nid = typeOne[i].ValueId
            N = kvMap['v'+typeOne[i].ValueId]
            ...
            for(var j = 0; j<typetwo.length; j++){
                ...
                Vid = typetwo[j].ValueId
                V = kvMap['v'+typetwo[j].ValueId]
                // var attrs = {};
