...
exports.getInfo = function (urlStr, callback) {
    ...
    try {
        if (urlInfo.host == 'item.meitun.com' || urlInfo.host == 'item.babytreegroup.hk') {
    ...
    getHtml('http://m.meitun.com/newapi/item/itemDetailService?mt=1&sku=' + goodsMainId + '&topicid=186510&topicType=&promotionId=&promotionType=&regcode=250&provinceId=&protocol=http&token=', timesTamp, function (body, err) {
        ...
        if ( !body.data.hasOwnProperty('skulist')) {
            itemInfo.Status = 'outOfStock';
            return callback(null, itemInfo);
        }

        var ep = new eventproxy();
        ep.after('getUrls', body.data.skulist.length, function () {
            itemInfo.Md5 = md5(JSON.stringify(itemInfo));
            callback(null, itemInfo);
            return;
        })

        itemInfo.ItemAttributes.Title = body.data.name;
        itemInfo.ItemAttributes.ImageUrl = body.data.imagebigurl[0];
        // 讲类型id和类型名称单独赋值
        var idItemAttr = {}
        if (body.data.hasOwnProperty('specs')) {
            ...
            body.data.specs.forEach(function (spaceItem) {//循环sku属性
                ...
                spaceItem.groupdetails.forEach(function (groupdetails) {
                    var z = spaceItem.groupid
                    var types = {}
                    var valueId = groupdetails.specid
                    idItemAttr[z] = spaceItem.groupname
                    idItemAttr[valueId] = groupdetails.specname
        ...
        if (groupid == -1 && specid == -1) {
            ...
            body.data.skulist.forEach(function (skulistItems) {
                ...
                attrs.Attr = [];
                attrs.Offers = [];
                // getPrice(skulistItems.sku, times, attrs,itemInfo, ep)
                var timesTamp = Date.parse(new Date());//建立时间戳
                var timesTampNew = timesTamp - 2 * 20000
                var defer = Q.defer();
                getHtml('http://m.meitun.com/newapi/item/itemDetailService?mt=2&sku=' + skulistItems.sku + '&topicid=186510&topicType=&promotionId=&promotionType=&regcode=250&platform=1&protocol=http', timesTampNew, function (body, err) {
                    if (!err) {
                        try {
                ...
                ll.forEach(function (aas) {
                    var val = {}
                    val.Nid = aas.groupid
                    val.N = idItemAttr[aas.groupid]
                    val.Vid = aas.specid
                    val.V = idItemAttr[aas.specid]
                    attrs.Attr.push(val)
                })
...
// function getPrice(skuId, times, attrs, itemInfo, ep) {

//     var timesTamp = Date.parse(new Date());//建立时间戳
//     var timesTampNew = timesTamp - times * 20000
//     var defer = Q.defer();
...
//                 // getPrice(skuId, times, attrs, itemInfo)
//             }
//         }
