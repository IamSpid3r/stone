...
function getItemInfo(params, callback) {
    ...
    for(var i = 0; i< itemInfo.Items.length; i++){//遍历Items
        // 两两组合的id
        var Vid =[]
        for(var j = 0; j< itemInfo.Items[i].attr.length; j++){//遍历attr
            var VidNum = itemInfo.Items[i].attr[j].Vid
            Vid.push(VidNum)
        }

        var listSkusVal = JSON.parse(listSkus[1])
        // var Unique = []
        for(var h = 0; h<listSkusVal.length; h++){
            var Unique = []
            if(listSkusVal[h].listSpec.indexOf(Vid)){
                Unique.push(listSkusVal[h].sku)
            }
            itemInfo.Items[h].Unique = 'meitun.' + goods_type_id + '-0-' + Unique
            ...
            var getPrice = function(){
                ...
                getHtml('item.meitun.com/itemDetail/3059882-0-07020402400301.html',function(body, err){
                    ...
                    try{
                        ...
                        itemInfo.Items[h].Offers.List.Price = '100'
