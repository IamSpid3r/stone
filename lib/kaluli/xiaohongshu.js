...
exports.getInfo = function (urlStr, callback) {
    ...
    getHtml(urlStr, function (body, err, response) {
        ...
        //查看是否有在售商家
        var sellerValues = [];
        for (var i = 0, n = skuOptions.length + 1, vn = 0; i < items.length; i++) {
            if (items[i].seller && items[i].seller.name) {
                var flag = 0;
                if (sellerValues) {
                    for (var j = 0; j < sellerValues.length; j++) {
                        if (sellerValues[j].Name == items[i].seller.name) {
                            flag = 1;
                            break;
                        }
                    }
                ...
                if (!flag) {
                    var value = {};
                    value.ValueId = (n) * 1000 + vn;
                    value.Name = items[i].seller.name;
                    sellerValues.push(value);
                    vn++
                }
            }
        }
        if (sellerValues) {
            var sellerVariation = {};
            sellerVariation.Id = skuOptions.length + 1;
            sellerVariation.Name = "商家";
            sellerVariation.Values = sellerValues;
            variations.push(sellerVariation);
        }
        ...
        getHtml(apiUrl, function (body, err, response) {
            ...
            items.forEach(function (i) {
                ...
                priceItems.forEach(function (pv) {
                    if (pv.id == i.id && pv.stock_status == 1) {
                        item.Offers[0].List[0].Price = pv.price.sale_price.price;
                    }
                });
                if(item.Offers[0].List[0].Price == "") {
                    return ;
                }
                ...
                //循环设置第三个属性  在售商家
                if (i.seller && i.seller.name) {
                    var attr = {};
                    variations.forEach(function (vv) {
                        if ("商家" == vv.Name) {
                            vv.Values.forEach(function (vvv) {
                                if (i.seller.name == vvv.Name) {
                                    attr.Nid = vv.Id;
                                    attr.N = vv.Name;
                                    attr.Vid = vvv.ValueId;
                                    attr.V = vvv.Name;
