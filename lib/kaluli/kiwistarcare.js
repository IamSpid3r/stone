...
exports.getInfo = function (urlStr, callback) {
  ...
  if (urlInfo.host == 'www.kiwistarcare.com') {
...
function getItemInfo(params, callback) {
  ...
  var title = $('.more-commodity-1').text().trim();
  var price = $('.goods-price ').text();
  if (price) {
    price = price.replace('CNY￥', '').trim();
  }
  var img = $('.jqzoom img').attr('src');
  if(img == undefined || img == ''){
      callback({
          "Errors":{
              'Code': 'Error',
              "Message": 'goods image not found'
          }
      });
      return ;
  }
  ...
  var itemInfo = {
    ...
    ItemAttributes: {
      Title: title,
      ShopName: 'kimistarcare',
      ShopId: 'kimistarcare',
      ImageUrl: 'www.kiwistarcare.com' + img,
      Tax: 1
    },
    Variations: [],
    Items: []
  ...
  // 获取商品数量，判断售罄
  var goodsNum = $('#data_goodsNumber').text();
  if(goodsNum == 0){
