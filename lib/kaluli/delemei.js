...
function getItemInfo(params, callback) {
    ...
    //     }
    // });
    var price = $('#center_column').find('#our_price_display').text();
    if (price) {
    ...
    var type = [];
    var itemInfo = {
        Unique: 'www.delemei.de.' + pid,
        Md5: '',
        Status: 'inStock',
        Url: itemUrl,
        ItemAttributes: {
            Title: title,
            ShopName: 'www.delemei.de',
            ShopId: 'www.delemei.de',
            ImageUrl: img,
            Tax: 1
        },
        Variations: [],
...
function getHtml(urlStr, callback) {
    request({url:urlStr,  timeout : 5000}, function (error, response, body) {
