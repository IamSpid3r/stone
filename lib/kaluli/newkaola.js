/**
 * Created by captern on 2017/10/27.
 */
...
function getItemInfo(params, callback) {
    ...
    var getShopMain = function(){
        var defer = Q.defer();
        getHtml('http://www.kaola.com/product/'+goods_id+'.html', function(body, err){
            if(err){
                callback({
                    "Errors":{
                        'Code': 'Error',
    ...
    // 添加包邮部分信息
    // getHtml('https://www.kaola.com.hk/product/ajax/queryPromotionTitle.html?goodsId='+goods_id+'&categoryId='+itemInfo.ItemAttributes.categoryId+'&t='+new Date().getTime(), function(body, err){
    //     if(err){
    //         callback({
    //             "Errors":{
    //                 'Code': 'Error',
