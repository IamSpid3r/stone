    ...
    try {
        ...
        skuDetailList.forEach(function (skuDetail, index) {
            ...
            if(currentStore != 0 && currentStore != '0'){
                ...
                //存在的id
                item.Attr.forEach((valTmp) => {
                    if (existsAttrId.indexOf(valTmp.Vid) == -1) {
                        existsAttrId.push(valTmp.Vid);
                    }
                })
            }
        })
        ...
        Variations.forEach(function (Variation, index) {
            ...
            for(var i=0;i<Values.length;i++){
                ...
                if ('ImageUrls' in Values[i] && !Values[i].ImageUrls[0]) {
                    delete Values[i].ImageUrls;
                }
                if(flag){
