...
function getItemInfo(params, callback) {
    ...
    try{
        ...
        //获取店铺名称和是否自营标签
        getHtml("https://goods.kaola.com/product/"+goods_id+".html", function(body, err){
            if(err){
                callback({
                    "Errors":{
                        'Code': 'Error',
        ...
        //税费
        getHtml('https://goods.kaola.com/product/getPcGoodsDetailDynamic.json?provinceCode=310000&goodsId=' + goods_id, function(body, err){
            if(err){
                callback({
                    "Errors":{
                        'Code': 'Error',
