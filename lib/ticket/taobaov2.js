...
async function getItemInfoAll(activityId, sellerId, callback) {
    ...
    var data_obj = {"uuid":activityId,sellerId,"queryShop":true,"originalSellerId":"","marketPlace":""};
    ...
    try {
        ...
        console.log('error2', apiData);

        if (apiData.ret[0].indexOf('SUCCESS') == -1) {
            return callback(body.ret[0]);
        }
...
function getSign(param) {
    return new Promise((resolve, reject) => {
        request('http://van.shihuo.cn:7891/?'+param.join("&"), function (error, response, body) {
            ...
            //call(arr.sign,arr.time,arr.xuid,arr.sid)
            return resolve(arr);
        });
    })
}
