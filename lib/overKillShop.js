...
class OverKillShop {
    ...
    _getConfig() {
        if (!this._config) {
            ...
            for (let i = this._config.attributes.options.length - 1; i >= 0; i--) {
                if (productIds.indexOf(this._config.attributes.options[i].products[0]) < 0) {
                    this._config.attributes.options.splice(i, 1)
                }
            }
        }
    ...
    _addVariations($) {
        ...
        const values = this._getConfig().attributes.options.map((item) => {
            return {
                ...
                Name: (this._itemInfo.ItemAttributes.Gender === men ? 'M' : 'W') + item.label,
            }
        })
    ...
    _updateBaseInfo($) {
        ...
        this._itemInfo.Unique = `${country}.${platform}.${id}`
    ...
    _getDefaultImages($) {
        let urls = []
        $('.product-img-box .item').each((key, div) => {
            urls.push($(div).find('img').attr('src'))
        })
        return urls
    }

    _getTitle($) {
        return $('.product-name h2').text().trim()
    }

    _getColor($) {
        const id = $('.product-sku .muted:nth-child(2) .middlegrey').text()
