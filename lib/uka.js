...
exports.getInfo = function (urlStr, callback) {
    ...
    if (urlInfo.host == 'cn.unserekleineapotheke.de') {
...
function getItemInfo(params, callback) {
    ...
    if (!body) {
        ...
        if (notFoundReg.exec(body)) {//not found
            var itemInfo = {
                Unique: 'cn.unserekleineapotheke.' + pid,
                Md5: '',
                Status: 'notFind',
                Url: itemUrl,
                ItemAttributes: {},
    ...
    var title = $('.product-name').text().trim();
    var price = $('.rmb-price color_333').text();
    if (price) {
        price = price.replace('€', '').trim();
    }
    var img = $('.product-img-zoom').find('img').attr('src');
    ...
    var itemInfo = {
        ...
        ItemAttributes: {
            Title: title,
            ShopName: 'unserekleineapotheke',
            ShopId: 'cn.unserekleineapotheke.de',
            ImageUrl: img,
            Tax: 1
        },
        Variations: [],
    ...
    detail = {
        ...
        "Offers": [{
            "Merchant": {
                "Name": "cn.unserekleineapotheke.de"
            },
            "List": [
                {
                    "Price": price,
