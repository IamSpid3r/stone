//这个类和 amazonUsa.js 一样  不一样的几个地方用 是 createProdAdvClient  ，Unique  ，货币类型
...
var getInfo = function(urlStr, callback) {
    ...
    prodAdv = createProdAdvClient(awsAccesskey[0], awsAccesskey[1], 'jp', {
        region: 'jp',
    ...
    prodAdv.call("ItemLookup", {
        ...
        var itemInfo = {
            Unique: 'jp.amazon.' + parentAsin,
            Md5: '',
            Status: 'inStock',
            Url: 'http://www.amazon.co.jp/gp/product/' + parentAsin + '/',
            ItemAttributes: {
                Title: '',
                ShopName: '日亚',
                ShopId: 'jp.amazon',
        ...
        if ('Errors' in ProductOriginalData.Items.Request) {
            if (ProductOriginalData.Items.Request.Errors.Error.Message.indexOf('ItemIdの値として無効です') > 0) {
                ...
                // })
                // return;
                // console.log('getItemInfoByWebPage');
        ...
        if (ProductOriginalData.Items.Item.ParentASIN != undefined && ProductOriginalData.Items.Item.ParentASIN != ProductOriginalData.Items.Item.ASIN) {
            getInfo('http://www.amazon.co.jp/gp/product/' + ProductOriginalData.Items.Item.ParentASIN, callback);
            return;
        }
        ...
        if ('Variations' in amzProductData && 'Item' in amzProductData.Variations && amzProductData.Variations.Item instanceof Array) {
            for (var i = 0; i < amzProductData.Variations.Item.length; i++) {
                ...
                var Offer = {
                    ...
                    "List": [{
                        Price: parseFloat(amzItem.Offers.Offer.OfferListing.Price.Amount),
                        Type: "JPY"
                    }]
                }
            ...
            var Offer = {
                ...
                "List": [{
                    Price: parseFloat(amzProductData.Variations.Item.Offers.Offer.OfferListing.Price.Amount),
                    Type: "JPY"
                }]
            }
            item.Offers.push(Offer);
            ...
            var Offer = {
                ...
                "List": [{
                    Price: parseFloat(amzProductData.Offers.Offer.OfferListing.Price.Amount),
                    Type: "JPY"
                }]
            }
            item.Offers.push(Offer);
        ...
        itemInfo.Variations.forEach(function(val){
            if(val.Name == '颜色' && typeof val.Values[0].ImageUrls != 'undefined'){
...
var getItemInfoByWebPage = function(parentAsin) {
    var defer = Q.defer();
    proxyRequest({
        url: 'http://www.amazon.co.jp/dp/' + parentAsin
        ...
        var patt = /function\(A, \$, imageBlockATF, cf\){([\s\S]*?)return data;/ig;
        ...
        for (i in itemAttrData.asinToDimIndexMapData) {
            ...
            for (var j = 0; j < itemAttrData.asinToDimIndexMapData[i].length; j++) {
                var attr = {
                    Nid: j + 1,
                    N: variationsName(itemInfo.Variations[j].Name),
        ...
        Q.all(asinPromise).then(function(data) {
            for (i in data) {
                ...
                var index = _.findIndex(itemInfo.Items, {
                        Unique: 'jp.amazon.' + amzProductData.ASIN
                    })
...
function getSingleItemOffersInfo(asin) {
    ...
    var awsAccesskey = getAssessKey();
    var prodAdv = createProdAdvClient(awsAccesskey[0], awsAccesskey[1], 'jp', {
        region: 'jp',
        host: 'ecs.amazonaws.jp'
    });
    prodAdv.call("ItemLookup", {
...
var variationsName = function(name){
    return  (name == 'Color' || name == '颜色')
        ? '颜色'
        : ((name == 'Size' || name == '尺寸')
        ? '尺码'
        : name)
