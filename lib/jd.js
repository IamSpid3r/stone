...
exports.getInfo = function(urlStr, callback) {
    ...
    getHtml('http://ware.m.jd.com/client.action?functionId=skuDetail&body={%22skuId%22%3A%22'+pid+'%22}&clientVersion=5.0.1&client=apple', function(body, err){
        if(err){
            callback({
                "Errors":{
                    'Code': 'Error',
...
function getItemInfo(params, callback) {
    ...
    if(body.code != 0){//抓取失败
        ...
        return '';
    }else if(!body.wareInfo.basicInfo.skuColorSize.colorSize){//下架
    ...
    getHtml('http://p.3.cn/prices/mgets?type=1&skuIds='+priceSkus.join(), function(bodyPrices, err){
        ...
        if(colorSkus.length > 0){
            ...
            colorSkus.forEach(function (colorSku) {
                getHtml('http://ware.m.jd.com/client.action?functionId=skuDetail&body={%22skuId%22%3A%22'+colorSku[1]+'%22}&clientVersion=5.0.1&client=apple', function(body, err){
                    if(err){
                        callback({
                            "Errors":{
                                'Code': 'Error',
