...
function getItemInfo(params, urlInfo, callback) {
   ...
   // return callback(body);
    ...
    var categoryId = wareInfo.basicInfo.category;
    var venderId = wareInfo.basicInfo.venderId;
    var shopId = wareInfo.shopInfo.shop ?  wareInfo.shopInfo.shop.shopId : 0;

    itemInfo.ItemAttributes.Title = wareInfo.basicInfo.name;
    itemInfo.ItemAttributes.CategoryId = categoryId;
    itemInfo.ItemAttributes.ImageUrl = wareInfo.basicInfo.wareImage[0].big.replace(/\.webp/g, "");
    //服务
    var day7 = false;
    if (wareInfo.shopInfo.shop != null) {
        itemInfo.ItemAttributes.ShopId = 'cn.jd.' + shopId;
            ...
            getHtml(apiUrl, function (body, err) {
                if (err) {
                    ...
                    try {
                        ...
                        //缺少标题
                        if (!$('title').text()) {
                            return reject(new Error('get html error'))
                        }
                        // if ($('#returnurl').val()) {
    ...
    //获取优惠券
    var couponUrl = `https://cd.jd.com/promotion/v2?callback=c&skuId=${pid}&area=1_72_2799_0&shopId=${shopId}&venderId=${venderId}&cat=${encodeURIComponent(categoryId.replace(/;/g,','))}`;
        ...
        getHtml(couponUrl, function (body, err) {
            if (!err) {
                try {
                    ...
                    body = JSON.parse(body);
                    if (body.skuCoupon.length > 0) {
                        body.skuCoupon.forEach(function (coupon) {
                            if (coupon.couponType == 1 || coupon.couponType == 2) {//满减
                                itemInfo.Coupon.List.push({
                                    "Id": coupon.key,
                                    "Amount": [coupon.trueDiscount, coupon.quota],
