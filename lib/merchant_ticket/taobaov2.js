...
var xuid = "336134075";
//cookie2=
var sid = "1b0f6a979014234c183a20cb94d7a93d";
...
exports.getInfo = function (urlStr, shopId, sellerId, sellerType, callback) {
    var urlInfo = url.parse(urlStr, true);
    if (urlInfo.query.id) {
        //淘宝
        if (sellerType == 'C') {
            getItemInfoV3(urlInfo, shopId, sellerId, sellerType, callback);
        } else {
...
function getItemInfo(urlInfo, shopId, sellerId , callback) {
    (async () => {
        try {
            if (shopId) {
...
//mtop.macao.market.activity.applycoupon.querycouponsfordetail
function getItemInfoV3(urlInfo, shopId, sellerId, sellerType, callback) {
    (async () => {
        try {
            if (shopId) {
                var result = await getTableTicket(shopId);
            ...
            var lat = "31.271719";
            var lng = "121.476006";
            var ttid = "10001401@taobao_android_8.1.10";
            // var ttid = "10001401@taobao_android_8.0.0";
            var features = "27";
            var v = "1.0";
            ...
            var t = Date.parse(new Date()) / 1000;
            var data_obj = {
                "detail_v": "3.1.8", "from": "detail", "isAsy": "1", "isAsyn": "1",
                "itemId": urlInfo.query.id,
                "pageType": "weex", "sellerId": sellerId, "sellerType": sellerType,
                "ttid": ttid
            };
            var data = JSON.stringify(data_obj);
            Get_xsign(data, xuid, t, utdid, appKey, lat, lng, api, v, sid, ttid, deviceId, features, async function (err, signParams) {
                var xsign = signParams.sign, signTime = signParams.time;
                var url = "http://guide-acs.m.taobao.com/gw/" + api + "/" + v + "/?type=originaljson&data=" + encodeURIComponent(data);

                xuid = signParams.xuid, sid = signParams.sid;
                headers = {
                    'x-appkey': appKey,
                    'x-t': signTime,
                    'x-pv': "5.2",
                ...
                async function c(error, response, body) {
                    ...
                    var coupons = info.data.coupons;
                    coupons.forEach(function (coupon) {
                        coupon.couponList.forEach(function (val) {
                            if (coupon.title == '店铺红包') {
                                var discountReg = /满([\.\d]*)(元{0,})可使用/.exec(val.subtitles[0]);
            ...
            function Get_xsign(data, xuid, t, utdid, appKey, lat, lng, api, v, sid, ttid, deviceId, features, call) {
                ...
                request('http://van.shihuo.cn:7891?' + param.join("&"), function (error, response, body) {
                    ...
                    //console.log("sign:"+arr.sign)
                    return call(error, {
                        sign: arr.sign, time: arr.time, xuid: arr.xuid, sid: arr.sid
                    })
                });
            }
        } catch (err) {
