...
function getItemInfo(urlInfo, callback) {
    ...
    request({
        ...
        if (responseFirstJson.data.seller.type == 'B' || 1) {
            proxyRequest({
                url: 'https://mdskip.taobao.com/core/initItemDetail.htm?household=false&cartEnable=true&service3C=false&itemId=' + urlInfo.query.id,
                headers: {
                ...
                for (i in responseFirstJson.data.skuModel.ppathIdmap) {
                    ...
                    var offer = {
                        ...
                        List: [{
                            Price: parseInt(responseSecondJson.defaultModel.itemPriceResultDO.priceInfo[responseFirstJson.data.skuModel.ppathIdmap[i]].price),
                            Type: 'RMB'
                        }]
                    };
