...
function getItemInfo(urlInfo, callback) {
    ...
    proxyRequest({
       ...
       // callback(null,[responseFirstJson,apiStackValue]);return ;
        ...
        var getTaxInfo = function() {

            var defer = Q.defer();
            var tax='';
            var pageUrl='';
            if(responseFirstJson.data.seller.type == 'B'){
                pageUrl='https://detail.m.tmall.com/item.htm?id=' + urlInfo.query.id
            }else{
           ...
           /* proxyRequest({
                ...
                headers: {
                    ...
                    'referer': 'https://detail.m.tmall.com/item.htm?spm=a220m.1000858.1000725.1.9bJPSD&id=' + urlInfo.query.id
                },
                encoding: null
                ...
                res = _DATA_Mdskip;
                var inter = res.vertical.inter;
                if(typeof(inter) == 'undefined'){
                    itemInfo.ItemAttributes.Tax = 1;
                }else{
                    if(inter.tariff.value == '商家承担' || inter.tariff.value.indexOf('包税')>=0){
                        itemInfo.ItemAttributes.Tax = 1;
                    }else{
        ...
        getTaxInfo().then(function () {
            ...
            itemInfo.Variations.forEach(function (variations, variationsIndex) {
                ...
                if (variations.Name == '颜色') {
                    variations.Values.forEach(function (colorArr, colorIndex) {
                        isOk = false;
                        itemInfo.Items.forEach(function (item) {
                            item.Attr.forEach(function (attr) {
                                if (attr.N == '颜色') {
                                    if (attr.V == colorArr.Name) {
                                        isOk = true;
                                    }
                                    return;
                                }
                        ...
                        if (!isOk) {
                            delete  itemInfo.Variations[variationsIndex].Values[colorIndex];
                        }
                    })
                    itemInfo.Variations[variationsIndex].Values = itemInfo.Variations[variationsIndex].Values.filter(function(val){
                        if(val) return val;
